<template>
  <v-form>
    <v-container>
      ...
      <div class="content">
        ...
        <v-row>
          ...
          <v-col class="inputForm" cols="12" md="4">
            <v-textarea label="코디명" auto-grow solo rows="1" v-model="name"></v-textarea>
            <v-textarea
              label="태그"
              auto-grow
              solo
              ...
              v-model="tag"
            ></v-textarea>
            <v-textarea solo label="설명" v-model="description"></v-textarea>
          </v-col>
        </v-row>
      </div>
    </v-container>
    ...
    <!-- 등록 버튼 start -->
    <v-btn icon id="add-click-btn" @click="clickCreate()">
